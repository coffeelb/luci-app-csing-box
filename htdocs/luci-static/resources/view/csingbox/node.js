...
function parseShareLink(uri, features) {
	...
	if (uri[0] && uri[1]) {
			...
			try {
				...
				try {
					...
					if (suri.length <= 2) {
						if (suri.length === 2)
							slabel = '#' + suri[1];
						uri[1] = cs.decodeBase64Str(suri[0]) + slabel;
					}
				} catch(e) { }
				...
				if (url.username && url.password) {
					...
					userinfo = cs.decodeBase64Str(decodeURIComponent(url.username)).split(':');
					if (userinfo.length > 1)
						userinfo = [userinfo[0], userinfo.slice(1).join(':')]
				}

				if (!cs.shadowsocks_encrypt_methods.includes(userinfo[0]))
					return null;
		...
		case 'vmess':
			...
			uri = JSON.parse(cs.decodeBase64Str(uri[1]));
...
function renderNodeSettings(section, data, features, main_node) {
	...
	s.modaltitle = L.bind(cs.loadModalTitle, this, _('Node'), _('Add a node'), data[0]);
	s.sectiontitle = L.bind(cs.loadDefaultLabel, this, data[0]);
	...
	o.load = L.bind(cs.loadDefaultLabel, this, data[0]);
	o.validate = L.bind(cs.validateUniqueValue, this, data[0], 'node', 'label');
	o.modalonly = true;
	...
	o.depends('type', 'hysteria2');
	o.validate = cs.validatePortRange;
	o.modalonly = true;
	...
	for (let i of cs.shadowsocks_encrypt_methods)
		o.value(i);
	/* Stream ciphers */
	o.value('aes-128-ctr');
	o.value('aes-192-ctr');
	...
	o.onchange = function(ev, section_id, value) {
		let desc = this.map.findElement('id', 'cbid.csingbox.%s.transport'.format(section_id)).nextElementSibling;
		if (value === 'http')
		...
		let tls = this.map.findElement('id', 'cbid.csingbox.%s.tls'.format(section_id)).firstElementChild;
		if ((value === 'http' && tls.checked) || (value === 'grpc' && !features.with_grpc)) {
			this.map.findElement('id', 'cbid.csingbox.%s.http_idle_timeout'.format(section_id)).nextElementSibling.innerHTML =
			...
			this.map.findElement('id', 'cbid.csingbox.%s.http_ping_timeout'.format(section_id)).nextElementSibling.innerHTML =
	...
	o.value('/etc/csingbox/certs/client_ca.pem');
	o.depends('tls_self_sign', '1');
	o.validate = cs.validateCertificatePath;
	o.rmempty = false;
	o.modalonly = true;
	...
	o.depends({'tls_self_sign': '1', 'tls_cert_path': '/etc/csingbox/certs/client_ca.pem'});
	o.onclick = L.bind(cs.uploadCertificate, this, _('certificate'), 'client_ca');
	o.modalonly = true;
	...
	o.value('/etc/csingbox/certs/client_ech_conf.pem');
	o.depends('tls_ech', '1');
	o.modalonly = true;
	...
	o.inputstyle = 'action';
	o.inputtitle = _('Upload...');
	o.depends({'tls_ech': '1', 'tls_ech_config_path': '/etc/csingbox/certs/client_ech_conf.pem'});
	o.onclick = L.bind(cs.uploadCertificate, this, _('ECH config'), 'client_ech_conf');
	o.modalonly = true;
	...
	if (features.with_utls) {
		...
		o.validate = function(section_id, value) {
			if (section_id) {
				let tls_reality = this.map.findElement('id', 'cbid.csingbox.%s.tls_reality'.format(section_id)).firstElementChild;
...
return view.extend({
	...
	render(data) {
		...
		for (let suburl of (uci.get(data[0], 'subscription', 'subscription_url') || [])) {
			...
			const urlhash = cs.calcStringMD5(suburl.replace(/#.*$/, ''));
		...
		s = m.section(form.NamedSection, 'subscription', 'csingbox', _('Node management'));
							...
							if (input_links && input_links[0]) {
								...
								input_links.forEach((l) => {
									...
									if (config) {
										...
										let nameHash = cs.calcStringMD5(config.label);
		...
		o.onclick = function() {
			return fs.exec_direct('/etc/csingbox/scripts/update_subscriptions.uc').then((res) => {
