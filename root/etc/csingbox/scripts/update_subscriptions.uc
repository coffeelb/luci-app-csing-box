...
const ubus = connect();
const sing_features = ubus.call('luci.csingbox', 'singbox_get_features', {}) || {};
/* Common var end */

/* Log */
system(`mkdir -p ${RUN_DIR}`);
function log(...args) {
	const logfile = open(`${RUN_DIR}/csingbox.log`, 'a');
	logfile.write(`${getTime()} [SUBSCRIBE] ${join(' ', args)}\n`);
	logfile.close();
}

function service_action(action) {
	return system([ '/etc/init.d/csingbox', action ]);
}
...
function main() {
	...
	if (!isEmpty(main_node)) {
		...
		if (first_server) {
			...
			log('No available node, service disabled.');
		}
	}
